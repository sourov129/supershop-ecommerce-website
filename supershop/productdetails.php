...
$query=$con->prepare("SELECT * FROM `products` WHERE id='$result'");

// $query=$con->prepare("SELECT * FROM `products` WHERE category='$cato'");
...
if(isset($_POST["quantity"])){
    ...
    foreach($list as $item){
        $query=$con->prepare("INSERT INTO `customercart`(`product_name`, `quantity`, `price`, `image`) VALUES ('$item[1]','$quantity','$item[3]','$item[7]')");
        $res=$query->execute();
    }

  // submit button a click kore redirect kore product dekhano
...
foreach($list as $item){
    echo "
    <div class=\"product-view\">
        <img src=\"$item[7]\" class=\"img-thumbnail\" style=\"height:450px; margin:30px;\" alt=\"\">
        <div class=\"itemdetails\">
        <h2>$item[1]</h2>
        <h1 style=\"font-size: 30px;\">৳ $item[3]</h1>
        <form action=\"\" method=\"POST\" onsubmit=\"buynow()\">
  <label for=\"quantity\">Quantity (between 1 and 5):</label>
  <input type=\"number\" id=\"quantity\" name=\"quantity\" min=\"1\" max=\"5\"><br><br>
  <input type=\"submit\" value=\"Buy Now\" class=\"btn-shop\" >
</form>
